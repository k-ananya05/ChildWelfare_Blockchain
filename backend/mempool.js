...
class ChildWelfareTransaction extends Transaction {
    ...
    validateChildWelfareRules() {
        ...
        if (this.action === 'update_record' &&
            !['NGO', 'Government', 'Hospital'].includes(this.from.split('_')[0])) {
            return false;
        }

        // Only government can transfer guardianship
        if (this.action === 'transfer_guardianship' && !this.from.includes('Government')) {
            return false;
        }
